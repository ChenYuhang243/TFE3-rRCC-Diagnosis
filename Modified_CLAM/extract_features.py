...
parser.add_argument('--level', type=str,default="10X")
parser.add_argument('--model_name', type=str, default='resnet50_trunc', choices=['resnet50_trunc', 'uni_v1', 'conch_v1',"gigapath","virchow","chief"])
...
if __name__ == '__main__':
    ...
    loader_kwargs = {'num_workers': 24, 'pin_memory': True} if device.type == "cuda" else {}

    total = len(bags_dataset)
    for bag_candidate_idx in range(total):
        slide_id = bags_dataset[bag_candidate_idx]
        bag_name = Path(slide_id).name +".h5"
        bag_candidate = slide_id
        ...
        dataset = Whole_Slide_Bag(file_path=file_path, img_transforms=img_transforms,level=args.level)
        ...
        if not Path(pt_path).parent.exists():
            Path(pt_path).parent.mkdir(parents=True)
